...
public class PlaylistLoader {
    ...
    public List<String> getPlaylistNames() {
        if (folderExists()) {
            ...
            return Arrays.asList(folder.listFiles((pathname) -> pathname.getName().endsWith(".txt"))).stream()
                ...
                manager.loadItemOrdered(name, items.get(i), new AudioLoadResultHandler() {
                    ...
                    public void trackLoaded(AudioTrack at) {
                        if (config.isTooLong(at))
                            errors.add(new PlaylistLoadError(index, items.get(index), "この曲は長すぎます。"));
                        else {
                            at.setUserData(0L);
                            tracks.add(at);
                            consumer.accept(at);
                    ...
                    public void noMatches() {
                        errors.add(new PlaylistLoadError(index, items.get(index), "何も見つかりませんでした。"));
                        done();
                    }
                    ...
                    public void loadFailed(FriendlyException fe) {
                        errors.add(new PlaylistLoadError(index, items.get(index),
                                "曲を読み込めませんでした: " + fe.getLocalizedMessage()));
                        done();
                    }
                });
            }
